...
app.on('ready', async () => {
  ...
  ipcMain.on(SAVE_PIPELINE_CHANNEL, savePipeline(mainWindow, db));

  ipcMain.on(ADD_PIPELINE_CHANNEL, addPipeline(mainWindow, db));

  ipcMain.on(DELETE_PIPELINE_CHANNEL, deletePipeline(mainWindow, db));
